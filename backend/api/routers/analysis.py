...
async def run_monitoring(request: MonitorRequest):
    """
    Execute comprehensive multi-agent portfolio analysis, quantitative risk calculation,
    and ML multivariate anomaly detection.
    """
...
async def get_risk_metrics():
    """Retrieve computed quantitative risk metrics."""
    if app_state.current_portfolio is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if app_state.latest_results and app_state.latest_results.get("risk_metrics"):
        return {
            ...
            "briefing": app_state.latest_results.get("briefing"),
            "trace": app_state.latest_results.get("trace"),
            "news_analyses": app_state.latest_results.get("news_analyses", []),
            "orchestrator_engine": app_state.latest_results.get("orchestrator_engine", "LangGraph StateGraph")
        }
...
async def get_active_alerts():
    """Retrieve active risk alerts for current holdings."""
...
async def get_portfolio_health():
    """Get portfolio health score breakdown (0-100)."""
