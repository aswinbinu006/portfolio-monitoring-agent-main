...
Strict type validation, sane defaults, and institutional fintech data representations.
"""
...
class APIResponse(BaseModel, Generic[T]):
    ...
    error: Optional[APIErrorDetail] = Field(default=None, description="Error detail if unsuccessful")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="UTC response generation time")
...
class HoldingItem(BaseModel):
    ...
    current_price: Optional[float] = Field(default=None, ge=0, description="Latest market price")
    current_value: Optional[float] = Field(default=None, ge=0, description="Total current position value")
    weight: float = Field(default=0.0, ge=0.0, le=1.0, description="Current weight in portfolio (0.0 to 1.0)")
    target_weight: Optional[float] = Field(default=None, ge=0.0, le=1.0, description="Target weight")
    sector: Optional[str] = Field(default="Diversified", description="Economic sector classification")
    daily_change_pct: Optional[float] = Field(default=None, description="Today's percentage change")
    unrealized_pnl: Optional[float] = Field(default=None, description="Unrealized profit/loss value")
    unrealized_pnl_pct: Optional[float] = Field(default=None, description="Unrealized profit/loss percent")
